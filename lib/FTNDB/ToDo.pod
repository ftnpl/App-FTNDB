...
Although the NAME entry is present, getting the following warning:
    $ perl Makefile.PL
    WARNING: Setting ABSTRACT via file 'lib/FTNDB.pm' failed
...
least initially having the name ftndbadm.
...
nodelist table; perhaps also add a configuration option for the same thing?
...
(If there are no hub nodes in a particular net, then effectively the hub
